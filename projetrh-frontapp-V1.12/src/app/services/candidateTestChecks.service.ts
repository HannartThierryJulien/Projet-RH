...
export class CandidateTestChecksService {
  ...
  public areSecurityChecksGood(candidateTest: CandidateTest, candidateId: number): boolean {
    ...
    if (this.isTestAlreadyCompleted(candidateTest)) {
      return false;
    }
  ...
  private isTestAlreadyCompleted(candidateTest: CandidateTest) {
    if (candidateTest.completionDate == null) {
      return false;
    } else {
      return true;
    }
  ...
  private isTestExpired(candidateTest: CandidateTest): boolean {
    ...
    let convertedAssignationDate = new Date(candidateTest.assignationDate);
